 ...
 */
class ilErrorTextWizardInputQuestionGUI extends ilTextInputGUI
{
        ...
        if (is_array($a_value)) {
            ...
            if (is_array($a_value['key'])) {
                foreach ($a_value['key'] as $idx => $key) {
                    array_push($this->values, new assAnswerErrorTextQuestion(
                        $key,
                        $a_value['value'][$idx],
    ...
    function insert($a_tpl)
    {
        ...
        $tpl = $this->plugin->getTemplate("tpl.prop_errortextquestionwizardinput.html");
        $i = 0;
